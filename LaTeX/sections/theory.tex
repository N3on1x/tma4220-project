...
We only state the formulas for evaluating integrals numerically on simplexes using barycentric quadrature points. For an in-depth cover of this topic, we refer to \cite{quarteroni}. Let $\mathcal{\hat{K}}$ be the reference simplex in $\mathbb{R}^n$ with corners $\vb{\hat{e}_1},\dots,\vb{\hat{e}_n}$ and $\vb{0}$. Each point in $\mathcal{\hat{K}}$ can then described with a barycentric coordinate $\vb{\pmb{\lambda}} = \left(\lambda_1,\dots,\lambda_{n+1}\right)$. Then 
